  ...
  Stream<String> parse(Response response) async* {
    ...
    for (var node in departuresElements) {
      for (var element in node.querySelectorAll("h2.pt.bt-light")) {
        ...
        var _dateString = element.text;

        for (var tr in node.querySelectorAll(".mt tbody tr")) {
          _flight.type = "departure";
          var _timeString = "${tr.querySelectorAll("td")[0].text}";
          _flight.datetimeStr = "$_dateString $_timeString";
          _flight.code = tr.querySelectorAll("td")[1].text;
          _flight.operator = tr.querySelectorAll("td")[2].text;
          _flight.destination = tr.querySelectorAll("td")[3].text;
          _flight.status = tr.querySelectorAll("td")[4].text;
